...
})(CardinalCompassDirection || (CardinalCompassDirection = {}));
...
})(OrdinalCompassDirection || (OrdinalCompassDirection = {}));
...
const height = ({ orientation, dimensions: { yRadius } }) => orientation === Orientation.POINTY ? heightPointy(yRadius) : heightFlat(yRadius);

const hexToPoint = ({ orientation, dimensions: { xRadius, yRadius }, origin: { x, y }, q, r }) => orientation === Orientation.POINTY
    ? {
...
const width = ({ orientation, dimensions: { xRadius } }) => orientation === Orientation.POINTY ? widthPointy(xRadius) : widthFlat(xRadius);
...
const isFlat = ({ orientation }) => orientation === Orientation.FLAT;

const isPointy = ({ orientation }) => orientation === Orientation.POINTY;
...
function line(options) {
    return (cursor, getHex) => {
        ...
        if (options.direction in CompassDirection) {
...
const ring = ({ start, at, center, rotation }) => (cursor, getHex) => {
    rotation = rotation?.toUpperCase() ?? Rotation.CLOCKWISE;
...
const rays = ({ at, start, length, rotation, updateRay = (_) => _, }) => {
    return (cursor, getHex) => {
        ...
        const ringStart = { q, r: r - length, s: s + length };
        return ring({ center: firstCoordinates, start: ringStart, rotation })(cursor, getHex)
            .reduce((uniqueHexes, through) => {
            const ray = line({ at, start, through })(cursor, getHex);
...
const spiral = ({ radius, start, at, rotation }) => (cursor, getHex) => {
    ...
    return branch(line({ start, at, direction: CompassDirection.N, length: radius }), ring({ center, rotation }))(getHex(center), getHex);
};

export { CardinalCompassDirection, Compass, CompassDirection, Grid, OrdinalCompassDirection, Orientation, Rotation, add, assertCubeCoordinates, branch, center, cloneHex, corners, cornersFlat, cornersPointy, createHex, createHexPrototype, defaultHexSettings, distance, equals, flatTraverse, height, heightFlat, heightPointy, hexToOffset, hexToOffsetFlat, hexToOffsetPointy, hexToPoint, inStore, isFlat, isHex, isPointy, line, neighborOf, neighborOfFlat, neighborOfPointy, offsetToCube, offsetToCubeFlat, offsetToCubePointy, pointToCube, rays, rectangle, ring, round, spiral, toString, width, widthFlat, widthPointy };
